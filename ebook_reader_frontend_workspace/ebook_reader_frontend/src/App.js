...
function App() {
  ...
  // PUBLIC_INTERFACE
  /**
   * Robust EPUB search logic:
   * - Results array is empty (except summary/warning) if no chapters could be extracted.
   * - Every search operation, including Fuse/manual, only operates on actually extracted chapter text.
   * - Any placeholder, stale, or fallback data is removed.
   * - Additional explicit logs at every major stage prove true contents of search arrays.
   * - UI only ever shows matches derived from real content or info/debug lines.
   */
  async function handleSearch(query) {
    setSearchQuery(query);
    setSearchResults([]); // Always reset before work

    // Case 1: Empty query: forcibly clear results
    if (!query || query.trim() === "") {
      setSearchResults([]);
      console.log("[SEARCH][STATE] Query empty. Cleared results.");
      return;
    }

    // Case 2: No book loaded: results must *never* reflect any stale or fake data
    if (!book || typeof book !== "object") {
      console.info("[SEARCH][STATE] No book loaded. Cannot search.");
      setSearchResults([
    ...
    // Case 3: Check spine is present and valid
    let spineItems = [];
    try {
    ...
    if (!spineItems.length) {
      console.warn("[SEARCH][STATE] Book has no valid chapters (no spine items present).");
      setSearchResults([
        { snippet: "[INFO] No book content available (no chapters detected)." }
      ]);
      return;
    }

    // Case 4: Normalize query
    const normalizedQuery = query.trim().toLowerCase();

    // Extraction phase - Only real chapters are indexed. No placeholder/fake content. All state is explicit.
    const extractedChunks = [];
    const skippedSpineSections = [];
    const allSpineRaw = [];
    const spineChunkMeta = [];
    ...
    for (let i = 0; i < spineItems.length; ++i) {
      ...
      try {
        if (!spineItem || typeof spineItem.load !== "function")
          throw new Error("Spine item missing or not load()able");
        ...
        try {
          ...
          // Fallback, if available, to textContent
          if (
            spineItem.contents &&
    ...
    // Logging at extraction: show exactly what's searchable
    ...
    if (extractedChunks.length === 0)
      console.warn("[SEARCH][DEBUG] All chapters failed or have no text. Skipped sections:", skippedSpineSections);
    ...
    // Always provide feedback summary: never a fake result
    const infoSummary =
      ...
      (skippedSpineSections.length > 0
        ...
        : "");
    const feedbackItems = [{ snippet: infoSummary }];
    ...
    // If nothing was indexed, ONLY show info and error line (never any button/phantom match)
    if (extractedChunks.length === 0) {
      setSearchResults([
        ...feedbackItems,
    ...
    // ONLY the real extracted chunks are indexed and searched
    ...
    console.log("[SEARCH][DEBUG] Searchable text docset, length =", docs.length, docs);

    // Fuse.js block: Only valid if extractedChunks.length > 0 at this point
    ...
    // Manual indexOf search, also, only on real content
    let manualResults = [];
    ...
    // Results array: ONLY genuine matches are shown. Never phantom, never stale.
